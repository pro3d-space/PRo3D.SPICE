...
let main argv = 
    ...
    let targetState = 
        match CooTransformation.getRelState "MARS" "SUN" "HERA" time.Value referenceFrame with
        | None -> failwith "could not get initial camera view"
        | Some targetState -> targetState 
    ...
    let animationStep () = 
        bodies |> Array.iter (fun b -> 
            ...
            match CooTransformation.getRelState b.name "SUN" observer time referenceFrame with
            | Some rel ->
                b.pos.Value <- rel.pos
